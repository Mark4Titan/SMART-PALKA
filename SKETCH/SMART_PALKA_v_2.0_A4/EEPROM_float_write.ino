...
void reads(){   
  ...
  if (Serial.available() > 0) {
    char inChar = Serial.read();
     deboR = millis();
    if (inChar == 'h') {
      Serial.println(F("| 'command' highlighted  parameter |"));
      Serial.println(F("| 'x520'      fload      Volt_Max  |"));
      Serial.println(F("| 'n280'      fload      Volt_Min  |"));
      Serial.println(F("| 'e705'      int        anWrite   |"));
      Serial.println(F("| 'l110'      int        LedMod    |"));
      Serial.println(F("| 's'         bool       SAVE      |"));
      }
    if (inChar == 's') {FS = 1;}
