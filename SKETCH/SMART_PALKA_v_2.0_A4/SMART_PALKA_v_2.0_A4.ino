...
#define LED_COUNT 68          // число светодиодов в кольце/ленте
#define LED_DT 13             // пин, куда подключен DIN ленты

int max_bright = 45;          // максимальная яркость (0 - 255)
boolean adapt_light = 0;       // адаптивная подсветка (1 - включить, 0 - выключить)

byte fav_modes[] = {45, 46, 47, 48, 49, 50, 51, 53, 54, 55, 56, 22, 25, 21, 7, 24, 29, 38, 52, 1, 10, 2, 3, 34, 42, 5, 8, 11,  27, 13, 26 };  // список "любимых" режимов 38 уменьш скорасть
...
volatile byte ledMode = 110; // відобразити заряд батареї при старті (110)
/*
  Стартовый режим
  0 - все выключены
  1 - все включены
...
///....................................................................///
///.........настройки...voltage test animation v6......................///

double Volt_Max,              // максимальное значение в вольтах (при подключении по юсби 5.2v) с условием смещение на сегменты
       Volt_Min;              // минимальное значение в вольтах (минимальное значение для li-on 2.8v) с условием смещение на сегменты
int     anWrite;              // analogWrite значение при максимальных вольтах

//2 палка має anWrite = 1013; і Volt_Max = 5.18
...
byte segm = 4,                      // количество сегментов (max - 6, min - 1)
     prop = 2,                      // количество пробелов между сегментами (min - 1)
     addi = 2;                      // количество составления ленты (min - 1, max - 2)
     
byte F_deboA_OUT = 200;             // авто вимкнення після F_deboA_OUT*0.5мс секунд     
#define Discharge 5
byte Anim = 0;
bool je;                     // флаг анімації перед авто вимкненням
bool AKB_Flag = 0; // запуск індикації АКБ
...
byte Led_Average;                   // Середня LED в одном сегменте /////////////////////////////////////   byte
...
volatile uint32_t deboD;
volatile uint32_t deHOLD;
bool Flag_dD = 0;
...
byte K_Max =  0; //корректор отсечки (2 = 0.2)
byte K_Min =  4; //корректор отсечки (4 = 0.4)
...
byte CP[][3] = {                    //color package
// R     G   B 
  {20,  5,   5},                   // = заливка при низком заряде (Красный)  
  {40, 0,   0},                   // = segment №1 (Красный) 
  {40, 10, 0},                   // = segment №2 (Оранжевый) 
  {20, 10, 0},                   // = segment №3 (Желтый)  
  {0,   40, 0},                   // = segment №4 (Зеленый)    
  {0, 65, 10},                   // = segment №5 
  {0, 10, 65},                   // = segment №6
};
...
#define time_again 200   //  настройка таймаута на удержание (по умолчанию 300 мс)
bool hold_UP_DO = 0;
bool vol_an = 0;
//----Mark_Button END----///
////sis_END///
...
void setup()
{
  ...
  Serial.println(F(""));
  Serial.println(F("//====developed and tested MARK. mail: markfamiliytitan@gmail.com====//"));
  Serial.println(F("//=========God of IT Uliana. mail: uliana.a2101@gmail.com  ==========//"));
  Serial.println(F("//============firmware version 2.0_A4  (25,12,2023)==================//"));
  Serial.println(F(""));
  Serial.println(F("| smart stick |"));  
  Serial.println(F("| setup help enter 'h' |"));
  Serial.println(F("| setup! |"));  
  Serial.println(F("")); 

  

  Volt_Max = EEPROM_float_read(0);
  Volt_Min = EEPROM_float_read(4);
   anWrite = EEPROM_float_read(8);  
  
 //if (Volt_Max == 0) {EEPROM_float_write(0, 5.18); Volt_Max = EEPROM_float_read(0);}
 // if (Volt_Min == 0) {EEPROM_float_write(4, 2.80); Volt_Min = EEPROM_float_read(4);}
 // if (anWrite  == 0) {EEPROM_float_write(8, 1013);   anWrite = EEPROM_float_read(8);}  
  
  
  Serial.print(F(" Volt_Max = "));Serial.println(Volt_Max);
  Serial.print(F(" Volt_Min = "));Serial.println(Volt_Min);
  Serial.print(F("  anWrite = "));Serial.println(anWrite);
  ...
  Volt_Max =  Volt_Max -(float)K_Max/10; 
  Volt_Min =  Volt_Min -(float)K_Min/10; 
  //////////auto_voltage_test////
...
void loop() {
 ...
 // if (digitalRead(10) == 0){//читаємо
  ...
  ///========================================================voltage_test_a_v6 IN=======================================================================////  
  if (power_supply == 0 && charge_end == 1 && Anim == 1){Anim = 9; Flag_dD = 1; Cop_Vol_in = 0;} //Serial.print("Anim = ");Serial.println(Anim);}
  if (power_supply == 1 && charge_end == 1 && Anim != 1){Anim = 1; Flag_dD = 1;                }// Serial.print("Anim = ");Serial.println(Anim);}
  if (Anim == 0){Anim = 1; Cop_Vol_in = 0;                                   }//Serial.print("Anim = ");Serial.println(Anim);}
  if (charge_end == 0 && Anim != 12){Anim = 11; Flag_dD = 1; Cop_Vol_in = 0; }//Serial.print("Anim = ");Serial.println(Anim);}
  if (charge_end == 1 && Anim > 12){Anim = 1; Cop_Vol_in = 0;                }//Serial.print("Anim = ");Serial.println(Anim);}

 // if (millis() - deboD >= 1000 && charge_end == 1 && power_supply == 1 && Anim == 12) {
 //   deboD = millis();
 //   Anim = 1; Cop_Vol_in = 0;
 //   Serial.print("Anim = ");Serial.println(Anim);
 // }
  
  
  if (Anim == 9) {Anim++;Flag_dD = 1; ledMode = 110;                         }//Serial.print("Anim = ");Serial.println(Anim);}
  if (Anim == 11){Anim++;Flag_dD = 1; ledMode = 110;                         }//Serial.print("Anim = ");Serial.println(Anim);}
  
  if (E0 && power_supply == 1){ledMode = 110;}
  if (E0 != 1 && power_supply == 1){digitalWrite(Discharge, 0);}  

 //авто возврат
  if (millis() - deboD >= 2000 && Anim > 0 && Flag_dD == 1) {
    deboD = millis(); 
                                                                           //Serial.println("Flag_dD");
    Flag_dD = 0;
                                                                           // Serial.print("Anim = ");Serial.println(Anim);
  ...
  switch (ledMode) { 
    ...
    case 45: one_color_all(90, 90, 90); LEDS.show(); break; //---ALL White
    ...
    case 53: one_color_all(200, 20, 0); LEDS.show(); break;     //
    case 54: one_color_all(255, 0, 60); LEDS.show(); break;     //
    case 55: one_color_all(5, 190, 220); LEDS.show(); break;     //
    case 56: one_color_all(40, 0, 255); LEDS.show(); break;     //
