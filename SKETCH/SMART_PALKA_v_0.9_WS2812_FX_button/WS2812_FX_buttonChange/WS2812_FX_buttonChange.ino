...
byte fav_modes[] = {2, 11, 14, 25, 27, 30, 41, 36, 37, 43};  // список "любимых" режимов
...
volatile byte press = 0;  // счетчик нажатий максимум 255, занимает 1 байт
volatile boolean hold_flag = 0; // контроль удержание нажатий


#define waiting_time  100 // настройка антидребезга (по умолчанию 100 мс) 
#define time_again 500   //  настройка таймаута на удержание (по умолчанию 500 мс)
...
void Log() {
  ...
  if (millis() - debounce >= time_again && press>0) {
    ...
    if (digitalRead(2) == 0 && hold_flag == 0){
      ...
      if (press == 2){
              if (--modeCounter <= num_modes) modeCounter = 4;
              ledMode = fav_modes[modeCounter];    // получаем новый номер следующего режима
              changeFlag = true;
              change_mode(ledMode);               // меняем режим через change_mode (там для каждого режима стоят цвета и задержки)
...
void change_mode(int newmode) {
  thissat = 255;
  switch (newmode) {
    case 0: one_color_all(40, 40, 40); LEDS.show(); break; //---ALL OFF
