...
double Volt_Max = 4.20,              // максимальное значение в вольтах (при подключении по юсби 5.2v) с условием смещение на сегменты
       Volt_Min = 2.8;              // минимальное значение в вольтах (минимальное значение для li-on 2.8v) с условием смещение на сегменты
...
void loop() {
  ...
  if (Lx_f == 1) {                        // если включена адаптивная яркость
    ...
    Serial.println(ledMode);}
     ...
     if (power_supply == 0 && showing == 70){showing = 0;ledMode = 110;}
     
     if (millis() - deboVolt >= 1500 && showing <= 2) { 
    deboVolt = millis();    
    showing++;}
   if (showing == 2) {ledMode = 45; if (power_supply == 0)showing = 70;else showing = 80;}
