...
void VOL (){
 byte poh; 
if (power_supply == 0)poh=60;else poh=20;
  
if (millis() - deHOLD >= 60000) {
    deHOLD = millis();
    poh = 0;
    }
...
///інверсія  
  ...
  R6=0;
   ...
   if (V_Current >= 1){R1=1; Rm1=LED_COUNT - Led_Average-prop-1;}//se1i = (float)NUMPIXELS - segment1*2-prop;  
...
// AKB_Flag=1;
 ...
 // CHARGING_MODE=1;
   if ( power_supply == 0 && charge_end == 1){AKB_Flag = 1;}
   if ( power_supply == 1){AKB_Flag = 0;}
  
  if (millis() - deboB >= 300 && CHARGING_MODE && AKB_Flag) {
    deboB = millis();
    CAE=1;
     R1=0;
     R2=0;
 ...
 if (millis() - deboA >= 300 && E0){
    deboA = millis(); 
   je = !je; //Serial.print(F("je=")); Serial.println(je);
   ...
   if (je == 1){
      F1=1; Fm1=Led_Average + prop;
      R1=1; Rm1=LED_COUNT - Led_Average-prop-1;
      G0=1;
   }
   
 //==авто викл IN==    
   if (--F_deboA_OUT == 0){digitalWrite(Discharge, 1);}
   Serial.print(F("auto shutdown 0/"));
   Serial.println(F_deboA_OUT); 
 //==авто викл OUT==
 } 
 ...
 byte Lx = 0;
  ...
  for (int i = 0 ; i < LED_COUNT; i++ ) {setPixel(i, 0, 0, 0);}
   ...
   for (int i = 0 ; i < Fm1 && i < LED_COUNT/addi; i++ ) {
     if (i < Fm1-prop){
       if(E0){setPixel(i, CP[0][0],CP[0][1],CP[0][2]);}
       else{setPixel(i, CP[1][0],CP[1][1],CP[1][2]);}}
