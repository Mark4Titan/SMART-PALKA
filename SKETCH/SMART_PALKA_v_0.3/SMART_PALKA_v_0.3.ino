...
void Oper_control(){
  if (LED_X[0]>0){
   Serial.print("||  Oper_control START");  
    ...
    Serial.println(F(""));
    Serial.print(F("||  buttons - ")); Serial.print(LED_X[10]);  //}
    Serial.println(F("  ||"));
    ...
    if ( LED_X[0] > 9){ 
     Serial.println("  ||  hold  ||"); 
      ...
      while (digitalRead(BUTTON_PIN) == HIGH ) {
        if (LED_X[0] == 10){          
          if (L_IN[1] != 0){LED_X[1]=LED_X[1]-multiplier; LED_X[5]=LED_X[1];}
          if (L_IN[2] != 0){LED_X[2]=LED_X[2]-multiplier; LED_X[6]=LED_X[2];}
          if (L_IN[3] != 0){LED_X[3]=LED_X[3]-multiplier; LED_X[7]=LED_X[3];}
