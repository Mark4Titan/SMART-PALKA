...
void Log() {
  ...
  if (millis() - debounce >= time_again && press>0) {
    ...
    if (digitalRead(2) == 1){
     ...
     if (press == 1 && hold_UP_DO == 0){Lx-=10; if (Lx < 2)Lx = 5; Lx_f = 1;}

     //if (press == 2){vol_an = 1;}

   if (press == 2){if (temporary_Flag == 0){temporary_Flag = 1; temporary = ledMode; VOL ();}Serial.print(F("| temporary =  "));Serial.println(temporary); ledMode = 110;}
