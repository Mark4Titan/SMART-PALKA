...
//  analogReference(INTERNAL); //міняємо опорноє напряженіє
...
void loop(){
 buton_control();
  ...
  if (LED_X[0] == 0 && LED_X[10] == 8)LED_X[0] = LED_X[10]-1;
  if (LED_X[0] == 0 && LED_X[10] == 9)LED_X[0] = LED_X[10]-1;
...
void Oper_control(){
 ...
 //  Serial.print("||  Oper_control START"); 
    ...
    if (LED_X[0] == 10 && LED_X[10] == 8){LED_X[0] = 1;}
    if (LED_X[0] == 10 && LED_X[10] == 9){LED_X[0] = 1;}
    ...
    if ( LED_X[19] == 0 ){//// 
     ...
     byte Col = 9;   //кількість програм 
   
     if (LED_X[0] == 1) {LED_X[10]++; if (LED_X[10] > Col) LED_X[10]=1;}
     if (LED_X[0] == 2) {LED_X[10]--; if (LED_X[10] < 1) LED_X[10]=Col;} 
    
     if (LED_X[0] == 3) {Serial.println(F(" EX "));}  //меню  
       
       
   Serial.println(F(""));
   Serial.print(F("||  Click - ")); Serial.print(LED_X[10]);  //}
   Serial.print(F("  ||  Click index- ")); Serial.print(LED_X[0]); 
   Serial.println(F("  ||")); 
    
      
     switch (LED_X[10]) {
      case 3: {LED_X[1] = 255; LED_X[2] = 0  ; LED_X[3] = 0  ;   LED_X[5] = 255; LED_X[6] = 0  ; LED_X[7] = 0  ; break;} //green
      case 1: {LED_X[1] = 255; LED_X[2] = 255; LED_X[3] = 0  ;   LED_X[5] = 255; LED_X[6] = 255; LED_X[7] = 0  ; break;} //yellow
      case 2: {LED_X[1] = 0  ; LED_X[2] = 255; LED_X[3] = 0  ;   LED_X[5] = 0  ; LED_X[6] = 255; LED_X[7] = 0  ; break;} //red
      case 7: {LED_X[1] = 0  ; LED_X[2] = 255; LED_X[3] = 255;   LED_X[5] = 0  ; LED_X[6] = 255; LED_X[7] = 255; break;} //purple
      case 5: {LED_X[1] = 0  ; LED_X[2] = 0  ; LED_X[3] = 255;   LED_X[5] = 0  ; LED_X[6] = 0  ; LED_X[7] = 255; break;} //blue
      case 6: {LED_X[1] = 255; LED_X[2] = 0  ; LED_X[3] = 255;   LED_X[5] = 255; LED_X[6] = 0  ; LED_X[7] = 255; break;} //turquoise
    //  case 3: {LED_X[1] = 255; LED_X[2] = 255; LED_X[3] = 255;   LED_X[5] = 255; LED_X[6] = 255; LED_X[7] = 255; break;} //white
      case 4: {LED_X[1] = 100; LED_X[2] = 100; LED_X[3] = 100;   LED_X[5] = 100; LED_X[6] = 100; LED_X[7] = 100; break;} //white
      
       
     // case 7: { break;} // policce
 ...
 // if (LED_X[10] > 0 && LED_X[10] < 7) Farba();
       ...
       switch (LED_X[10]) {
         ...
         case 6: {Farba();  break;}
         case 7: {Farba();  break;}
         case 8: {LED_X[9]++; Farba_police(); break;}
         case 9: {LED_X[9]++; Farba_rainbow();  break;}
  ...
  //    bat = bat/100; 
      ...
      //bat =  (num1+num2)/2        
        ...
        //bat = bat*100;
           ...
           bat = map(bat*100, 0, 500, -10, num2+pix2);/// нижній поріг 2,8V Максимум 4,2V
...
void Farba_rainbow(){
  /*Serial.print("||  Farba_rainbow START"); 
   ...
   for(long firstPixelHue = 0; firstPixelHue < 3*65536; firstPixelHue += 256) {
    ...
    Serial.print("|| LED_X[9] = "); Serial.println(LED_X[9]);
   ...
   for (int i=0; i < num1; i++) {       
      ...
      pasek2.setPixelColor(i, pasek2.Color(255, 0, 0));
  }
  pasek.show();
  pasek2.show();
  delay(60);
...
void Farba_police(){
Serial.print("||  Farba_police START"); 
...
for (int j=0; j < 3; j++) {
   for (int i=0; i < num2; i++) {       
      pasek2.setPixelColor(i, pasek2.Color(0, 0, 255));
    //  pasek.setPixelColor(i, pasek.Color(0, 0, 255));
  }
  pasek2.show();
//  pasek.show();
  delay(60);
...
Serial.println("  Farba_police END   ||");  
}
...
Serial.print("||  LED_X[18] = "); Serial.println(LED_X[18]);
  ...
  for (int i=0; i < num2; i++) { 
       ...
       // pasek2.setPixelColor(i, pasek.Color(0, 0, 0));
        pasek2.setPixelColor(i, pasek2.Color(0, 0, 0));     
      delay (50);
       // pasek.show();
        pasek2.show();
      ...
      for (int i=0; i < LED_X[18]+1; i++) {
        ...
        for (int j=0; j < 55; j++) {
        if (i==0){pasek2.setPixelColor(i, pasek2.Color(j, 0, 0));
                  pasek2.setPixelColor(i, pasek2.Color(j, 0, 0));}        
        if (i==1){pasek2.setPixelColor(i, pasek2.Color(j, j, 0));
                  pasek2.setPixelColor(i, pasek2.Color(j, j, 0));}
        if (i==2){pasek2.setPixelColor(i, pasek2.Color(j, j, 0));
                  pasek2.setPixelColor(i, pasek2.Color(j, j, 0));}
        if (i==3){pasek2.setPixelColor(i, pasek2.Color(j, j, 0));
                  pasek2.setPixelColor(i, pasek2.Color(j, j, 0));}
        if (i>3){pasek2.setPixelColor(i, pasek2.Color(0, j, 0));
